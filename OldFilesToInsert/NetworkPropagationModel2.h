...
public:
    ...
    virtual void UpdateModel(const Real &R, int Type,
    ...
    virtual SufficientStatisticsVector GetSufficientStatistics(const Real& R, const Data& D);
...
protected:
    ...
    VectorType GetPropagationCoefficients(const Real& R);
    ...
    void ComputeInterpolationCoefficients(const Real& R);
    
    /// Compute Orthonormal Basis vec<B1, ..., B(N-1)> where Bi is vec<Ns>
    void ComputeOrthonormalBasis(const Real& R); 

    /// Compute the A Matrix used to get the space shifts
    void ComputeAMatrix(const Real& R); 

    /// Compute the space shifts
    void ComputeSpaceShifts(const Real& R);
