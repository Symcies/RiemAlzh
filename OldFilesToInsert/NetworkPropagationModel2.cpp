              ...
              const std::vector<std::string> Names) 
{
    ...
    auto R1 = std::make_shared<Real>(R);
...
NetworkPropagationModel2
::GetSufficientStatistics(const Real& R,
                          const Data& D) 
{
    //// Initialization
    std::shared_ptr<ExponentialCurveManifold> CastedManifold = std::static_pointer_cast<ExponentialCurveManifold>(m_Manifold);
...
::SaveData(unsigned int IterationNumber, const Real& R) 
{
    ...
    Outputs.open("Outputs/Network2/Parameters.txt", std::ofstream::out | std::ofstream::trunc);
...
NetworkPropagationModel2
::GetPropagationCoefficients(const Real& R) 
{
...
void
NetworkPropagationModel2
::ComputeInterpolationCoefficients(const Real& R) 
{
