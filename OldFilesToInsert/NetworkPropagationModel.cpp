...
void 
NetworkPropagationModel
::UpdateModel(const Real &R, int Type,
...
#pragma omp parallel for reduction(+:LogLikelihood)   
    for(size_t i = 0; i < N; ++i)
    {
        ...
        VectorType ParallelCurve = ComputeParallelCurve(P0, Delta, Nu, SpaceShift, TimePoint);
...
NetworkPropagationModel
::GetSufficientStatistics(const Real& R, const Data& D) 
{
    /// 
    VectorType P0(1, exp(R.at("P0")(0)));
...
void 
NetworkPropagationModel
::SaveData(unsigned int IterationNumber, const Real& R) 
{
    unsigned int NumberOfSubjects = R.at("Tau").size();
    std::ofstream Outputs;    
    Outputs.open("Outputs/Network/Parameters.txt", std::ofstream::out | std::ofstream::trunc);    
...
NetworkPropagationModel
::GetSubjectTimePoint(const int SubjectNumber, const Real& R) 
{
...
void
NetworkPropagationModel
::ComputeInterpoCoeffDelta(const Real& R) 
{
    /*
...
void
NetworkPropagationModel
::ComputeInterpoCoeffNu(const Real& R) 
{
    /*
...
void
NetworkPropagationModel
::ComputeOrthonormalBasis(const Real& R) 
{
    /// Get the data
...
void 
NetworkPropagationModel
::ComputeSpaceShifts(const Real& R) 
{
