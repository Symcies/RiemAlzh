...
::AlgorithmSettings(char* XMLFile) 
{
    ...
    m_MaximumNumberOfIterations = atoi(Settings->FirstChildElement("max-iterations")->GetText());
    m_NumberOfBurnInIterations = atoi(Settings->FirstChildElement("burn-in")->GetText());
    m_CounterToDisplayOutputs = atoi(Settings->FirstChildElement("step-size-to-display")->GetText());
